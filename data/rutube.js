...
self.port.on("detach", function(data) {
    if (data !== "") {
        ...
        var ouritem = document.getElementById("SEND_TO_DUNE");
        if (ouritem) {
            ouritem.remove();
        }
    }
...
function addSendButton(data) {
    ...
    if (addto.length > 0) {
        ...
        for (var n = 0; n < loadplag; n++) {
            document.getElementsByClassName('ByJoy')[n].style.float = 'left';
        }
        ...
        if (servers.length > 1) {
            ...
            serverlist.id = 'selectDUNEserver';
        ...
        a.onclick = function() {
            var serversend = {};
            var servselect = document.getElementById('selectDUNEserver');
            if (servselect) {
